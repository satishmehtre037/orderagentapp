...
export const ClinicForm: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-surface border border-line rounded-xl p-5 shadow-xs space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-line">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-sm font-semibold text-fg">Doctor Consultations & OPD Tariffs</h3>
          </div>
          <button
            type="button"
            ...
            >
              <div className="sm:col-span-5">
                <div className="flex items-center justify-between mb-1 sm:block">
                  <label className="block text-[11px] font-medium text-fg-muted">Consultation / Treatment</label>
              ...
              <div className="grid grid-cols-2 gap-2 sm:contents">
                ...
                <div className="sm:col-span-3">
                  <label className="block text-[11px] font-medium text-fg-muted mb-1">Slot Duration</label>
                  <input
      ...
      <div className="bg-surface border border-line rounded-xl p-5 shadow-xs space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-line">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-sm font-semibold text-fg">Doctors & Specialists</h3>
          </div>
          <button
            type="button"
            onClick={() => appendStaff({ name: '', specialty: 'Doctor' })}
          ...
          {staffFields.map((field, index) => (
            <div key={field.id} className="p-3 bg-surface-subtle rounded-lg border border-line space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-[11px] font-medium text-fg-muted">Doctor #{index + 1}</span>
              ...
              <input
                ...
                placeholder="Doctor Name (e.g. Dr. Rajesh Sharma, MD)"
                className="w-full text-xs font-medium px-3 py-2 bg-surface text-fg placeholder:text-fg-subtle border border-line rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent"
              />
              <input
                {...register(`staff.${index}.specialty` as const)}
                placeholder="Specialty (e.g. General Physician / Cardiologist)"
      ...
      {/* 3. OPD Clinic Timings */}
      <div className="bg-surface border border-line rounded-xl p-5 shadow-xs space-y-3">
        <div className="flex items-center space-x-2">
          <Clock className="w-4 h-4 text-accent" />
          <h3 className="text-sm font-semibold text-fg">OPD Clinic Timings</h3>
        </div>
        <input
          {...register('hours')}
          placeholder="e.g. Monday to Saturday: 9:00 AM - 1:00 PM & 5:00 PM - 9:00 PM"
      ...
      <div className="bg-surface border border-line rounded-xl p-5 shadow-xs space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-line">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-sm font-semibold text-fg">Patient FAQs</h3>
          </div>
          <button
            type="button"
          ...
          {faqFields.map((field, index) => (
            <div key={field.id} className="p-3 bg-surface-subtle rounded-lg border border-line space-y-2">
              ...
              <input
                ...
                placeholder="Question (e.g. Is prior appointment mandatory?)"
                className="w-full text-xs font-medium px-3 py-2 bg-surface text-fg placeholder:text-fg-subtle border border-line rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent"
              />
              <input
                {...register(`faqs.${index}.answer` as const)}
                placeholder="Answer (e.g. Walk-ins are accepted, but prior booking is recommended.)"
                className="w-full text-xs px-3 py-2 bg-surface text-fg placeholder:text-fg-subtle border border-line rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent"
              />
            </div>
          ))}
